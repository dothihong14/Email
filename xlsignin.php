...
if (!empty($hoten) && !empty($email) && !empty($matkhau) && !empty($xacnhanmatkhau)) {
    ...
    if ($result->num_rows > 0) {
        ...
        } elseif ($row['trangthai'] == 1 && sendConfirmationEmail($email, $hoten, $token)) {
            $_SESSION['message'] = "Email xác nhận đăng ký đã được gửi đến hộp thư của bạn, vui lòng kiểm tra!";
        }
        $check_stmt->close();
        $conn->close();
        header("Location: signin.php");
        ...
        // Tạo mã token
        $token = uniqid(bin2hex(random_bytes(16)), true); // Tạo token ngẫu nhiên, dài 32 ký tự

        // Chèn dữ liệu vào cơ sở dữ liệu (bao gồm token)
        $stmt = $conn->prepare("INSERT INTO user (hoten, email, matkhau, trangthai, token) VALUES (?, ?, ?, 1, ?)");
        if (!$stmt) {
        ...
        $stmt->bind_param("ssss", $hoten, $email, $hashed_password, $token);
        if ($stmt->execute()) {
            // Gửi email xác nhận
            if (sendConfirmationEmail($email, $hoten, $token)) {
                $_SESSION['message'] = "Email xác nhận đăng ký đã được gửi đến hộp thư của bạn, vui lòng kiểm tra!";
            } else {
...
function sendConfirmationEmail($recipientEmail, $recipientName, $token) {
    ...
    try {
        ...
        // Gọi hàm getHtmlContent và truyền token vào
        $mail->Body = getHtmlContent($recipientName, $recipientEmail, $token);
...
function getHtmlContent($hoten, $email, $token) {
    // Tạo đường link xác nhận
    $confirmationLink = "http://localhost:8888/Email-main/KT2/confirm.php?email=" . urlencode($email) . "&token=" . urlencode($token);
